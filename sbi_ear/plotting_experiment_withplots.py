...
from simulations.parameter_set_script import param_set
...
params = {
    'embedding': ['shallow', 'deep', 'shallow', 'shallow', 'deep', 'deep', 'deep', 'deep', 'deep', 'shallow',
                  'deep', 'deep', 'shallow', 'deep', 'shallow', 'shallow', 'shallow', 'deep', 'deep', 'deep'],
    'flow': ['MAF'] * 20,
    'transforms': [7, 5, 3, 3, 7, 5, 3, 7, 3, 3, 5, 5, 3, 5, 7, 3, 3, 3, 3, 7],
    'hidden_features': [512, 256, 256, 512, 512, 512, 256, 256, 256, 256, 512, 512, 256, 256, 512, 512, 512, 512, 256, 512],
    'hidden_features_no': [3, 3, 5, 3, 5, 3, 3, 5, 3, 3, 5, 3, 5, 5, 3, 5, 3, 3, 3, 3],
    'activation': [torch.nn.ELU] * 20,
    'optimizer': ['AdamW'] * 20,
    'init_lr': [1e-05, 0.0001, 1e-05, 1e-05, 1e-05, 0.0001, 1e-05, 1e-05, 0.0001, 0.0001,
                 0.001, 0.0001, 0.0001, 0.0001, 0.0001, 0.001, 0.0001, 1e-05, 0.001, 0.001],
    'weight_decay': [0, 0.0001, 0.001, 0.01, 0.0001, 0.0001, 0.0001, 0.01, 0.01, 0.01,
                      0.01, 0.01, 0.001, 0.001, 0, 0, 0.001, 0, 0.01, 0.01],
    'scheduler': ['ReduceLROnPlateau'] * 20,
    'min_lr': [1e-06, 1e-05, 1e-05, 1e-06, 1e-05, 1e-05, 1e-06, 1e-05, 1e-05, 1e-06,
                1e-08, 1e-08, 1e-07, 1e-08, 1e-07, 1e-08, 1e-08, 1e-07, 1e-08, 1e-07],
    'patience': [32, 16, 16, 32, 32, 32, 16, 16, 16, 16, 32, 32, 32, 32, 32, 16, 32, 32, 32, 32],
    'epochs': [1200] * 20,
    'stop_criterion': ['early'] * 20,
    'batch_size': [512, 512, 512, 1024, 512, 2048, 512, 2048, 512, 2048,
                    1024, 512, 2048, 256, 2048, 2048, 256, 2048, 1024, 2048],
    'spectral_length': [6144] * 20,
    'factor': [0.3, 0.5, 0.5, 0.3, 0.3, 0.3, 0.3, 0.7, 0.5, 0.7,
                0.3, 0.7, 0.7, 0.7, 0.3, 0.5, 0.7, 0.3, 0.7, 0.7],
    'noise': ['lognormaldist'] * 20,
    'gradient_steps_train': [1024] * 20,
    'gradient_steps_valid': [256] * 20,
    'ndim': [16] * 20
}
...
d = Data( path= '/home/mvasist/Highres/Sam/obs/DENIS_J0255_ergscm2nm/data_to_fit.dat')
processing = ProcessSpec(d)

sim = SpectrumMaker(wavelengths=d.model_wavelengths, param_set=param_set, lbl_opacity_sampling=2)
d_old = Data( path= '/home/mvasist/Highres/Sam/obs/spectrum_obs_old_ergscm2nm/data_to_fit.dat')


def simulator(theta):
    values_actual = theta[:-4].numpy()
    values_ext_actual = theta[-4:].numpy()
    _, spectrum, _ = sim(values_actual)
...
data_err = d.err 
data_flux_scaling = d_old.flux_scaling

@job(array=20, cpus=2, gpus=1, ram='32GB', time='10-00:00:00')
def experiment(index: int) -> None:
    ...
    # index = 15
    config = {
            'embedding': params['embedding'][index],  
            'flow': params['flow'][index],  
            'transforms': params['transforms'][index],  
            'hidden_features': params['hidden_features'][index],  
            'hidden_features_no': params['hidden_features_no'][index],  
            'activation': params['activation'][index],  
            'optimizer': params['optimizer'][index],  
            'init_lr': params['init_lr'][index],  
            'weight_decay': params['weight_decay'][index],  
            'scheduler': params['scheduler'][index],  
            'min_lr': params['min_lr'][index],  
            'patience': params['patience'][index],  
            'epochs': params['epochs'][index],  
            'stop_criterion': params['stop_criterion'][index],  
            'batch_size': params['batch_size'][index],  
            'spectral_length': params['spectral_length'][index],  
            'factor': params['factor'][index],  
            'noise': params['noise'][index],  
            'gradient_steps_train': params['gradient_steps_train'][index],  
            'gradient_steps_valid': params['gradient_steps_valid'][index],  
            'ndim': params['ndim'][index]
            }
    ...
    def noisy(theta, x, b= None): #50 is 10% of the median of the means of spectra in the training set.
        bs = x.size()[0]
        # data_uncertainty = Data().err * Data().flux_scaling
        data_uncertainty = data_err * data_flux_scaling
    ...
    class NPEWithEmbedding(nn.Module):
        def __init__(self):
            ...
            if config['flow'] == 'NCSF':
                self.npe = NPE(
                    16, self.embedding.out_features,
                    # moments=((l + u) / 2, (l - u) / 2),
    ...
    class plots(): 
        ...
        def __init__(self, runpath, ep, estimator): #statepath
            self.runpath = runpath   
            self.ep = ep
            self.estimator = estimator
            # self.statepath = statepath

            self.savepath_plots = runpath  / ('plots_' + str(self.ep) + '_SamM2data7224')
            ...
            states = torch.load(runpath / ('states_' + str(ep) + '.pth'), map_location='cpu')
        ...
        def coverage(self): 
            ...
            with torch.no_grad():
                for theta, x in tqdm(islice(testset, 2**7)): #**8
                    theta, x = torch.hstack((theta[:,:10], theta[:,14:])).cuda(), x.cuda()
        ...
        #### Corner plot
            ...
            valid_mask_npe = torch.isfinite(self.theta).all(dim=1)
            self.theta = self.theta[valid_mask_npe]

            corner_fig = corner_mod([self.theta[:2991]], legend=['NPE'], \
                            color= ['steelblue'] , figsize=(16,16), \
                        domain = (LOWER, UPPER), labels= LABELS, \
                            labelsize = 16, legend_fontsize = 18,\
        ...
        def ptprofile(self):
            ...
            fig_pt = PT_plot(fig, ax, self.theta[:2**8, :-1], pressures, invert = True, \
    ...
    #         
        ...
        def sim_spectra(self, theta, theta_name, x_name, only_returning = True, isnoisy = True):
            if not only_returning:
                ...
                x = x * (d_old.flux_scaling/d.flux_scaling)
    ...
    #     start = time.time()
    #     losses = torch.stack([ step( pipe(torch.hstack((theta[:,:10], theta[:,14:])).float(), x.float()) ) for theta, x in islice(trainset, 1024) ]).cpu().numpy()
    #     ## 770, 1024, 256
    #     end = time.time()
    ...
    #     with torch.no_grad():            
    #         losses_val = torch.stack([ pipe(torch.hstack((theta[:,:10], theta[:,14:])).float(), x.float()) for theta, x in islice(validset, 256) ]).cpu().numpy()
    #     #90 256 32
    #     run.log({
...
#####################################################################
    ...
    epoch = config['epochs']
    runpath = savepath / name
    # runpath = savepath / (name + '_SamM2data7224')
    runpath.mkdir(parents=True, exist_ok=True)
