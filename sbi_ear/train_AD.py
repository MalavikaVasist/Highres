...
class NPEWithEmbedding(nn.Module):
    def __init__(self):
        ...
        #         6144*2 , 128, hidden_features=[512] * 2 + [256] * 3 + [128] * 5,  #1291 #6144, 3072, 1536
...
def train(i: int):
    ...
    # Run
    run = wandb.init(project='highres',  config = config_dict, name = 'ancient-destroyer-24-1') #+CausalConv
    ...
    estimator = NPEWithEmbedding().cuda() #double()
    ...
    optimizer = optim.AdamW(estimator.parameters(), lr= 0.0000001953125, weight_decay=1e-2)
    ...
    plot = plots(runpath, int((epoch+1350)/50) * 50)
